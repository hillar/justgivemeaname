...
export class AuthFreeIPA extends AuthBase {
  constructor (server=HOST, base=BASE, binduser=BINDUSER, bindpass=BINDPASS, field=FIELD, rejectUnauthorized=false, directory,filename, time, logger) {
  ...
  async reallyVerify (username,password) {
    ...
    if (!(u.length === 1)) {
      this.log_notice({'notuser':{username,filter}})
    ...
    const fu = new User(ru.uid, ru.roles, ru.groups, ru.ssn, ru.fn, ru.ln, ru.ou , ru.manager , ru.emails , ru.phones, this._logger)
